...
- **The RO-root/persistent-data slice is hardware-accepted.** The board
  ...
  `/data` is the direct `MICROPANEL_DATA` ext4 mount. A marker and SSH host
  keys remained stable through normal reboot and physical power-cycle; all
  core units remained active with no failed units. The app
  ...
  acceptance. The earlier stable `machine-id` result came from a baked lower-
...
3. **Display-class HAL seam made real** (pulled from Sprint 5, and the enabler for the capacitive panel): the existing `DisplayBackend`/`TouchInput` selection resolves a small **panel-profile** concept — a named profile bundles {image boot configurator, geometry, touch driver signature + quirks, backlight control path}. The app has portrait/landscape ADS7846 PiScreen profiles plus the named Luckfox 3.5-RPi-LCD-CTP portrait profile. Its GT911 at I²C `0x5d` was live-probed (ID `911`, firmware `1060`) and fresh-image accepted on 2026-08-14: ST7796S DRM `/dev/fb0`, kernel `gpio-backlight`, GT911 Type-B MT, portrait interaction, calibration, and HMI/broker clean boot all passed. Type-B MT position/slot/tracking-ID axes bypass the ADS7846 `BTN_TOUCH` guard; the input layer keeps the first complete contact as the primary pointer and releases it rather than jumping to a second finger. **Owner decision (2026-08-14): retain separate PiScreen and `--variant=luckfox-ctp` images, rather than a post-flash profile switch.** Their SPI0/reset claims are mutually exclusive, and the appliance keeps `/boot/firmware` read-only, so a persistent boot-profile change is an image-build concern rather than a runtime UI setting. The Luckfox variant installs the vendor-pinned ST7796S MIPI-DBI command sequence, exact Goodix `addr=0x5d` overlay, and `panel_mipi_dbi` module-load rule. Every future electrically distinct SPI panel gets its own explicitly accepted variant; this is a deliberate support matrix, not runtime detection.
4. **RO-rootfs vertical slice on the real image** (pulled from Sprint 5's item 3, minimal form): rootfs + `/boot/firmware` read-only, tmpfs overlay with `recurse=0`, small writable `data` partition mounted `nofail`; `PersistentStorage` writes go there with the durable-commit protocol; corrupt/absent `data` falls back to tmpfs defaults. NetworkManager connection profiles are a `data`-backed bind mount and SSH host keys are seeded on `data` before SSH starts. The app must reject an overlay/tmpfs `data` directory rather than silently storing durable state there. This is where `ExecutionContext`'s production roots (`data_dir`, `log_dir` on `data`; `runtime_dir` on tmpfs) first run for real. Power-cut soak is deferred to Sprint 5; the *shape* is proven here so no menu item is written against writable-root assumptions. **Machine identity is now an explicit early-boot disposition:** the lower image contains empty identity files; systemd creates a random first-boot ID, and a root-only `/data/micropanel-touch-system/machine-id` store captures it before `systemd-machine-id-commit`, D-Bus, NetworkManager, SSH, and the HMI begin. Later boots restore that device-specific value. PID 1 necessarily starts before `/data`, but no consumer receives a build-chroot ID. Fresh-image, reboot, and hard-power-cycle acceptance remain required.
5. **Display power and brightness management** (moved out of Sprint 3, see §7): the verified Luckfox implementation is hardware-accepted for basic sleep/wake on 2026-08-14. The read-only `power.display_sleep_sec` config key supplies a factory default (60 seconds; `0` disables), while **Display → Standby** stages a user override in `display-settings.conf`: checkbox plus a 10–180 s slider in 10-second steps. Its Apply commits changes and Back discards them; disabled state visibly greys the slider. The Display page uses a three-row/two-column grid so its five icon-labelled tiles stay within the 320×480 portrait viewport. The Luckfox-only boot profile deliberately changes the MIPI-DBI backlight to kernel-owned PWM on GPIO 18, exposing `backlight_pwm/brightness`. **Display → Brightness** changes the hardware as the 5–100% slider is dragged and saves the final value on release in `display-brightness.conf`, mapped to the closest discrete PWM level; it needs only a Back button, and standby restores the selected value on wake. PiScreen enables neither control: GPIO 22 belongs to the panel overlay's pin group, and the 2026-08-15 three-second low-drive probe made no visible backlight change. This accepted bench clone is fixed-on. A profile-gated controller blanks the backlight, pauses only LVGL display refresh/invalidation, and restores/repaints it on first evdev activity. The input layer consumes every report through the wake contact's release, so that gesture cannot operate a control. Active flash/destructive jobs inhibit full sleep and wake an already blank display. Udev grants the appliance account `rw` access only to the selected profile's backlight attribute at node creation, avoiding asynchronous-panel-probe races. The managed Luckfox PWM profile explicitly disables analogue audio, which shares the competing PWM resource. Power measurement and the long-action acceptance remain required before this item exits.
...
**Exit demo:** the PiScreen and Luckfox `--variant=luckfox-ctp` image builds each boot **read-only** to the app as the appliance service; a static-IP change applies through the broker on the real image; the Luckfox display sleeps after 60 s and wakes on touch without activating a control; both accepted panels are demonstrated; pull power at a random moment and each image boots back cleanly.
...
6. **Touch-calibration rescue screen** (PRD requirement, sol-review-v1 F-13) — implemented: the System-menu flow takes five numbered target taps, fits a residual axis-aligned X/Y transform, rejects degenerate, inconsistent, or implausible fits, and atomically writes a versioned `touch-calibration.conf` below the app's persistent data root. It is loaded only when its geometry and evdev ranges match the active panel, otherwise the default mapping is retained. Re-running the flow replaces the prior correction; a two-tap **Reset default** control durably removes it and restores the in-memory factory mapper immediately. SSH removal followed by service restart remains break-glass recovery. This applies to resistive profiles; capacitive profiles normally retain the default mapping.
...
- **Trigger:** `lv_display_get_inactive_time()` is checked once per second; the read-only config supplies the factory `display_sleep_sec` default (60; `0` disables), then a validated persisted user setting may override it. The UI exposes 10–180 seconds in 10-second steps; any evdev activity resets it.
- **Sleep:** backlight off through the profile's kernel-exported path — on the accepted Luckfox image, `/sys/class/backlight/backlight_pwm/brightness`. GPIO/PWM access remains kernel-owned; udev grants the HMI account access only to the selected attribute when it is created. LVGL invalidation and its display-refresh timer pause, while the input/timing path continues at a reduced 100 ms cadence. The PWM profile restores the selected brightness level after wake, or the kernel-reported maximum if no usable prior level exists. Three consecutive failed backlight transitions emit one diagnostic and suppress automatic retries; a user input or an explicit standby-settings change re-arms the controller. On the 2026-08-15 bench PiScreen, a three-second GPIO 22 low drive had no visible effect, so its fixed-on backlight enables neither sleep nor the 5–100% brightness control. The backlight LED is the dominant panel power draw; DRM DPMS panel-sleep can be layered later if measurements justify it.
