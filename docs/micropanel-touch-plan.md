...
3. **Display-class HAL seam made real** (pulled from Sprint 5, and the enabler for the capacitive panel): the existing `DisplayBackend`/`TouchInput` selection resolves a small **panel-profile** concept — a named profile bundles {boot configuration, geometry, touch driver signature + quirks, backlight control path}. The app has portrait/landscape ADS7846 PiScreen profiles plus the named Luckfox 3.5-RPi-LCD-CTP portrait profile. Its GT911 at I²C `0x5d` was probed live (ID `911`, firmware `1060`); Type-B MT position/slot/tracking-ID axes bypass the ADS7846 `BTN_TOUCH` guard while LVGL consumes the lowest active contact as one pointer. The default image remains the verified PiScreen profile. The `--variant=luckfox-ctp` image path is deliberately separate because the two SPI panels have mutually exclusive display/reset GPIO claims; it installs the vendor-pinned ST7796S MIPI-DBI command sequence and Goodix overlay. Fresh-image on-panel acceptance remains required before calling the Luckfox profile released.
