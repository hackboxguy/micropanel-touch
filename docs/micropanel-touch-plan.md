...
**Exit demo:** the PiScreen and Luckfox `--variant=luckfox-ctp` image builds each boot **read-only** to the app as the appliance service; a static-IP change applies through the broker on the real image; the display sleeps after 60 s and wakes on touch without activating a control; both accepted panels are demonstrated; pull power at a random moment and each image boots back cleanly.
...
- **Sleep:** backlight off through the profile's kernel-exported path — on the accepted Luckfox image, `/sys/class/backlight/backlight_pwm/brightness`; on PiScreen, `/sys/class/leds/micropanel-touch-piscreen-backlight/brightness`. GPIO/PWM access remains kernel-owned; udev grants the HMI account access only to the selected attribute when it is created. LVGL invalidation and its display-refresh timer pause, while the input/timing path continues at a reduced 100 ms cadence. The PWM profile restores the selected brightness level after wake, or the kernel-reported maximum if no usable prior level exists; PiScreen's GPIO-22 node restores its binary enabled level. The PiScreen wiring has no variable brightness capability, so the 5–100% control remains unavailable there. The backlight LED is the dominant panel power draw; DRM DPMS panel-sleep can be layered later if measurements justify it.
