...
5. **Display power management** (moved out of Sprint 3, see §7): implemented for the verified Luckfox profile on 2026-08-14. The `power.display_sleep_sec` config key defaults to 60 seconds (`0` disables); a profile-gated controller blanks the kernel-owned `backlight_gpio/brightness`, pauses only LVGL display refresh/invalidation, and restores/repaints it on the first evdev activity. The input layer consumes every report through the wake contact's release, so that gesture cannot operate a control. Active flash/destructive jobs inhibit full sleep and wake an already blank display. The Luckfox image alone grants the appliance account `rw` access to that single sysfs attribute through an ordered root one-shot unit; the PiScreen profile remains unsupported because it has no verified kernel backlight path. **Physical sleep/wake acceptance and power measurement remain required before this item exits.**
...
- **Trigger:** `lv_display_get_inactive_time()` is checked once per second; threshold from config (`display_sleep_sec`, default 60, `0` disables). Any evdev activity resets it.
