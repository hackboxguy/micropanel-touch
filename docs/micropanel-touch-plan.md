...
5. **Display power management** (moved out of Sprint 3, see §7): the verified Luckfox implementation is hardware-accepted for basic sleep/wake on 2026-08-14. The read-only `power.display_sleep_sec` config key supplies a factory default (60 seconds; `0` disables), while **System → Display Standby** persists a user override in `display-settings.conf`: checkbox plus a 10–180 s slider in 10-second steps. Slider changes apply on release; checkbox changes apply immediately; disabled state visibly greys the slider. A profile-gated controller blanks the kernel-owned `backlight_gpio/brightness`, pauses only LVGL display refresh/invalidation, and restores/repaints it on first evdev activity. The input layer consumes every report through the wake contact's release, so that gesture cannot operate a control. Active flash/destructive jobs inhibit full sleep and wake an already blank display. The Luckfox image alone grants the appliance account `rw` access to that single sysfs attribute through an ordered root one-shot unit; the PiScreen profile remains unsupported because it has no verified kernel backlight path. Power measurement and the long-action acceptance remain required before this item exits.
