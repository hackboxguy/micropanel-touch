...
3. **Display-class HAL seam made real** (pulled from Sprint 5, and the enabler for the capacitive panel): the existing `DisplayBackend`/`TouchInput` selection now resolves a small **panel-profile** concept — a named profile bundles {overlay line, geometry, touch driver signature + quirks, backlight control path}. The app has portrait/landscape ADS7846 profiles plus capacitive Type-B multitouch profiles for the same 320×480/480×320 geometries: MT position/slot/tracking-ID axes select the latter, bypassing the ADS7846 `BTN_TOUCH`-ordering guard while LVGL consumes the lowest active contact as one pointer. The current image remains on the verified resistive PiScreen profile. The exact capacitive boot overlay is deliberately pending a named, physical Goodix/FT5x06-class panel and its verified wiring; do not invent it. Once proven, add it to the profile and image selector, then accept the same image + selected profile on both panels without a rebuild.
