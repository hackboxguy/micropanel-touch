...
5. **Display power and brightness management** (moved out of Sprint 3, see §7): the verified Luckfox implementation is hardware-accepted for basic sleep/wake on 2026-08-14. The read-only `power.display_sleep_sec` config key supplies a factory default (60 seconds; `0` disables), while **Display → Display Standby** stages a user override in `display-settings.conf`: checkbox plus a 10–180 s slider in 10-second steps. Its Apply commits changes and Back discards them; disabled state visibly greys the slider. The Luckfox-only boot profile deliberately changes the MIPI-DBI backlight to kernel-owned PWM on GPIO 18, exposing `backlight_pwm/brightness`. **Display → Brightness** stages and persists a 5–100% value in `display-brightness.conf`, mapped to the closest discrete PWM level; standby restores that value on wake. A profile-gated controller blanks the backlight, pauses only LVGL display refresh/invalidation, and restores/repaints it on first evdev activity. The input layer consumes every report through the wake contact's release, so that gesture cannot operate a control. Active flash/destructive jobs inhibit full sleep and wake an already blank display. The Luckfox image alone grants the appliance account `rw` access to that one sysfs attribute through an ordered root one-shot unit; the PiScreen profile remains unsupported because it has no verified variable-backlight path. The PWM route uses the analogue-headphone resource, so analogue audio is unavailable on this specific variant. Power measurement and the long-action acceptance remain required before this item exits.
