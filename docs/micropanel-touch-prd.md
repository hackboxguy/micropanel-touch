...
- **Menu presentation is config + skin data, never code:**
  - **Per-menu layout** via the optional additive `layout` key: `"list"` (default single-column rows) or `"grid"`, with optional `columns` (default 2). Grid items flow row-major into square-ish tiles sized from `content_width / columns`, scrolling when they overflow — 2×2 and 2×3 arrangements are simply item count × column count, in either orientation.
