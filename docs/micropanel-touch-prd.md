...
- **Unknown keys are ignored** (forward compatibility), and new touch-only keys are **optional and additive** (`icon`, `accent`, `layout`, `input`), so one JSON file can drive both products for as long as the OLED variant lives. Ignoring unknown keys is *not* the same as accepting broken structure — see validation below.
